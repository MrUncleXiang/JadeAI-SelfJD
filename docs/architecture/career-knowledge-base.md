...
职业事实继续保持原子化：项目、技能、经历等分别审核，避免把多个来源和不同生命周期的
陈述绑定成一个无法独立追溯的大对象。为避免原子事实数量较多时形成卡片堆叠，知识库使用
固定高度的滚动表格作为审核工作台，右侧详情区展示完整内容、证据、Claim 和结构化数据。
工作台支持关键词、类型和状态筛选，以及当前可见 Draft 的全选或多选批量批准/拒绝：
...
单条或批量审核成功后，前端在当前列表中原位更新状态，不重新拉取并重排队列；当前行、
滚动位置和详情选择保持稳定。已处理项在本次审核会话中暂时保留，用户可主动应用筛选条件，
或切换筛选/刷新页面后再隐藏，避免连续审核时频繁寻找下一条事实。
...
12. PAT 密文不能跨用户或跨连接解密；连接、仓库、同步和撤销 API 不返回令牌，401 会删除
    密文并停止后续同步。
13. 单条和批量审核在浏览器中原位更新，不额外请求事实列表；审核后的当前行、详情选择和
    滚动工作区保持可见，用户可主动应用筛选条件再收起已处理项。
