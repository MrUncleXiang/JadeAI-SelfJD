...
Phase 6A 已实现文字 JD 的首个端到端切片；Phase 6B.1 实现单张图片 JD；Phase 6C.1
实现已确认 JD 与 Approved Fact 共同约束的定向简历最小闭环：
...
- 已确认的 JD 卡片提供“生成定向简历”入口；用户可从 Approved Fact 新建，或选择已有
  Resume Version 复制为独立 Targeted Resume。
- 定向生成只把当前用户的 Approved Fact Evidence 和当前已确认 JD Requirement 加入引用白名单，
  输出必须先进入 Resume Change Set 审阅，不能直接覆盖简历。
- 为兼容响应较慢或不擅长复杂联合 Schema 的 OpenAI-compatible 网关，定向生成采用两阶段协议：
  LLM 只返回精简的职业摘要、技能组、项目及请求内短引用（如 `F01` / `J01`）；服务端将短引用
  解析为真实 Evidence / JD Requirement ID，再生成 Section/Item ID、目标哈希和 ResumePatch
  Operation。模型不再负责复制内部 UUID 或计算哈希。
- Targeted Resume 记录 `parent_resume_id` 和 `target_jd_source_id`；创建或模型提案失败时会回收空壳，
  应用前会重新验证事实和 JD 仍然有效。

尚未实现：PDF、DOCX 导入，图片原始二进制的长期对象存储，以及把 Requirement 与 Approved Fact
显式展示为 strong、partial、gap、conflict 的可人工调整匹配矩阵。当前定向 Change Set 已在 Operation
级保存 `evidenceIds` 与 `jdRequirementIds`，但这不替代独立的缺口分析页面。
...
当前 Phase 6C.1 已实现上述最小闭环。用户没有基准简历时，系统会从 Approved Fact 创建空白的
Targeted Resume 骨架；选择基准简历时复制指定或最新 Version。两种路径都先生成 Change Set，
进入编辑器审阅后才可应用。显式匹配矩阵仍属于后续 Phase 6C.2。

当前模型输出是受限的 `TargetedResumeDraft`，包含摘要、技能组、项目和每块内容对应的
`factRefs` / `jdRequirementRefs` 短引用。服务端先将它们映射为批准的 `evidenceIds` /
`jdRequirementIds` 并验证引用白名单，再确定性转换为 ResumePatch：

- 摘要生成 `set_field`；
- 与基准副本中同名的技能组或项目生成 `update_item`；
- 其他技能组或项目生成 `add_item`；
- `operationId`、新 Item ID 和 `expectedHash` 均由服务端生成；
- 基准副本中的其他真实内容默认保留，不由模型直接删除。
...
- 未确认 JD、跨租户基准简历和没有 Approved Fact 的请求被拒绝且不创建空壳。
- 定向 Operation 只能引用当前 JD 的 Requirement；JD 被重新编辑而失去确认后，旧提案应用失败关闭。
- 模型无法提供合法 Draft、引用越权或超时时会返回稳定错误码，并删除刚创建的 Targeted Resume。
- Draft 到 ResumePatch 的转换只使用服务端生成的 ID 和 Hash；同名基准项目更新时保留日期、URL 等未改字段。
- 模型提案失败时删除刚创建的 Targeted Resume。
