...
- 为兼容响应较慢或不擅长复杂联合 Schema 的 OpenAI-compatible 网关，定向生成采用两阶段协议：
  LLM 只返回精简的职业摘要、技能组、项目及双重引用；服务端再生成 Section/Item ID、目标哈希和
  ResumePatch Operation。模型不再负责复制内部 ID 或计算哈希。
...
`evidenceIds` / `jdRequirementIds`。服务端验证引用白名单后，确定性转换为 ResumePatch：
