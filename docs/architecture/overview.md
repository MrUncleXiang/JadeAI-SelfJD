...
WorkResume schemaVersion 2 浏览器目录上传、无凭证公共 GitHub URL 和用户级加密
Fine-grained PAT 私有仓库；GitHub App 作为需要安装授权和 Webhook 时的可选高级入口。
...
- PAT 连接创建、可访问仓库读取、仓库选择和撤销；同步本身进入后台 Job。
- LLM 档案 CRUD。
- 查询同步、导入和导出状态。
