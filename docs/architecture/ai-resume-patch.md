...
- 定向生成使用 `targeted-resume-draft-v2-short-refs` 两阶段协议：模型返回较小的摘要/技能/项目
  Draft，并只引用本次请求内的 `F01` / `J01` 短引用；服务端把短引用解析为真实 Evidence / JD
  Requirement ID，验证双重引用后确定性生成 ResumePatch 的 Operation ID、Item ID 与 `expectedHash`。
