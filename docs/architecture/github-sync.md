...
公共 URL、PAT 和 App 用户都可点击“检查更新”。手动任务与 Webhook、定时任务共享相同
幂等键，不能并行创建同一 Commit 的多个同步任务。上传来源通过重新上传检查内容摘要，
不伪装成远端自动同步。
...
| 公共仓库连接 | `POST /api/github/public-connections`（计划） |
| PAT 私有仓库连接 | `POST /api/github/pat-connections`（计划） |
