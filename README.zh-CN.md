...
> **二开状态：** 当前分支正在实施 JadeAI Career 方案。账号密码、管理员、数据库会话和
> 迁移验收已进入 Phase 1；用户级加密 LLM 档案、GitHub App 同步和 AI 简历变更协议将在
> 后续阶段实现。上游公开 Docker 镜像不包含这些二开改动。
...
> **AI 配置：** 当前兼容 UI 仍在浏览器保存配置；Phase 2 会迁移为服务端加密、按用户隔离的多 LLM 档案。
...
当前兼容 UI 允许用户配置 OpenAI-compatible、Anthropic 或自定义端点，API Key 暂存在浏览器 `localStorage`。Phase 2 会迁移为服务端加密、按用户隔离的多 LLM 档案，并清理旧浏览器存储。
