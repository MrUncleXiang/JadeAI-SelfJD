...
export class GitHubPatClient {
  ...
  async getAuthenticatedUser(): Promise<GitHubAuthenticatedUser> {
    const body = record(await this.request('/user'));
    return { id: identifier(body.id), login: string(body.login) };
  }
  ...
  async listRepositories(): Promise<GitHubRepository[]> {
    ...
    for (let page = 1; page <= 100; page++) {
      const body = await this.request(
        `/user/repos?per_page=100&page=${page}&sort=full_name&direction=asc`,
      );
      if (!Array.isArray(body)) throw new GitHubApiError('INVALID_RESPONSE', 502);
      const current = body.map(parseRepository);
      repositories.push(...current);
      if (current.length < 100) return repositories;
    }
    throw new GitHubApiError('RESPONSE_TOO_LARGE', 502);
  ...
  async getRepository(repositoryId: string): Promise<GitHubRepository> {
    return parseRepository(await this.request(`/repositories/${encodeURIComponent(repositoryId)}`));
  }
