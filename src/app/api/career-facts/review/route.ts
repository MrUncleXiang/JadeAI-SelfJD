...
const batchReviewSchema = z.object({
  factIds: z.array(z.string().trim().min(1).max(200)).min(1).max(100),
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const input = await readAuthJson(request, batchReviewSchema);
    const facts = await careerService.reviewFacts(actor, input.factIds, input.decision, input.note);
    const response = NextResponse.json(facts.map(toCareerFact));
